 ...
 * @author DEI-ESINF
 */
public class BST<E extends Comparable<E>> implements BSTInterface1<E> {
     ...
     */
    //se não houver raiz não há árvore
    @Override
    ...
    private Node<E> insert(E element, Node<E> node1) {
        if (node1== null) {
            return new Node(element, null, null);
        }

        //se o elemento a ser introduzido for menor que o que lá está coloco-o à esquerda
        if (node1.getElement().compareTo(element) > 0) {
            node1.setLeft(insert(element, node1.getLeft()));
            //se o elemento a ser introduzido for maior que o que lá está coloco-o à direita
        } else if (node1.getElement().compareTo(element) < 0) {
            node1.setRight(insert(element, node1.getRight()));
        } else {
    ...
    private Node<E> remove(E element, Node<E> node) {
        ...
        if (element.compareTo(node.getElement()) == 0) {
            ...
            //quando há mais do que uma subárvore. Optou-se pelo valor mais pequeno da árvore da direita
    ...
    private int size(Node<E> node) {
        if (node == null) {
            ...
            //size da esquerda + 1(raíz) + size da direita
    ...
    protected E smallestElement(Node<E> node) {
        ...
        Node<E> current = node;
        //percorre a árvore toda da esquerda porque assim encontrará o menor elemento
    ...
    private void processBstByLevel(Node<E> node, Map<Integer, List<E>> result, int level) {
        if (node != null) {
            //se o nível não existir cria-se uma nova arraylist para o nível
            if (result.get(level) == null) {
                result.put(level, new ArrayList<E>());
            }

            //se já existir vamos buscar o level e adicionar o elemento à frente
            result.get(level).add(node.getElement());
            processBstByLevel(node.getLeft(), result, level + 1);//mais 1 porque está a descer de nível
